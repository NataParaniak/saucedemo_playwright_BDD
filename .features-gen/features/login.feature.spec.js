...
test.describe("Login", () => {
  test("Successful login with valid credentials", async ({
    ...
    await Then("the user should be redirected to the Inventory page", null, {
      page,
    });
  });
  ...
  test("Login with a locked-out user", async ({
    ...
    await And("the user clicks the Login button", null, { page });
    await Then(
      'the error message "Epic sadface: Sorry, this user has been locked out." should be displayed',
      null,
      { page },
    );
  ...
  test("Login with empty credentials", async ({ Given, Then, And, page }) => {
    ...
    await And("the user clicks the Login button", null, { page });
    await Then(
      'login error message "Epic sadface: Username is required" should be displayed',
      null,
      { page },
    );
    await And("user should remain on the login page", null, { page });
    ...
    steps: [
      {
        ...
        gherkinStepLine: 4,
        keywordType: "Context",
        textWithKeyword: "Given the user is on the SauceDemo login page",
        stepMatchArguments: [],
      },
      {
        pwStepLine: 8,
        gherkinStepLine: 5,
        keywordType: "Action",
        textWithKeyword: "When the user enters valid credentials",
        stepMatchArguments: [],
      },
      {
        pwStepLine: 9,
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        gherkinStepLine: 12,
        keywordType: "Action",
        textWithKeyword: "When the user enters blocked credentials",
        stepMatchArguments: [],
      },
      {
        pwStepLine: 16,
      ...
      },
      {
        ...
        gherkinStepLine: 15,
        keywordType: "Outcome",
        textWithKeyword: "And user should remain on the login page",
        stepMatchArguments: [],
      },
    ],
  },
